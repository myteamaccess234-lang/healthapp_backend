...
router.post('/subscribe', verifyToken, async (req, res) => {
  ...
  try {
    // Attach userId to the subscription record
...
// 2. Trigger or send a push notification (with DB tracking and 20-min snooze support)
router.post('/send-notification', verifyToken, async (req, res) => {
  ...
  try {
    // Step A: Create notification document matching notificationModel.js schema
    ...
    // Step B: Build payload containing _id (crucial for quick actions like yes-water / no-forgot)
    ...
    if (subscriptions.length === 0) {
      return res.status(200).json({ 
        success: true, 
        message: 'Notification saved to DB, but no active push device subscriptions were found for this user.',
        notificationId: notificationRecord._id 
      });
    }

    // Step C: Send push with automatic cleanup for expired devices
    const pushPromises = subscriptions.map(sub =>
      webpush.sendNotification(sub, payload).catch(async err => {
        if (err.statusCode === 410 || err.body?.includes('unsubscribed or expired')) {
          console.log('Expired push subscription detected. Automatically removing...');
